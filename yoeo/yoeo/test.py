...
def print_eval_stats(metrics_output, seg_class_ious, class_names, verbose):
    ...
    print(class_names['segmentation'])
...
def write_eval_stats(metrics_output, seg_class_ious, class_names, fps):

    result = {'model': 'YOEO_default'}

    if metrics_output is not None:
        precision, recall, AP, f1, ap_class = metrics_output
        for i, c in enumerate(ap_class):
            result[class_names['detection'][c]] = AP[i]
        result['mAP'] = AP.mean()
    else:
        print("---- mAP not measured (no detections found by model) ----")

    for i, iou in enumerate(seg_class_ious):
        result[class_names['segmentation'][i]] = iou

    now = datetime.now()

    result['mIoU'] = np.array(seg_class_ious).mean()
    result['fps'] = fps
    result['batch_size'] = 8
    result['data'] = 'torso_reality_test'
    result['time_measurement'] = now.strftime("%H:%M:%S")

    json_object = json.dumps(result, indent=4)

    with open(f"../{result['model']}_{result['data']}_{result['time_measurement']}.json", "w") as outfile:
